...
impl FromStr for Components {
    ...
    fn from_str(v: &str) -> Result<Self, Self::Err> {
        static REGEX: Lazy<Regex> = Lazy::new(|| Regex::new(r"^(V)?,?\s?(S)?,?\s?(M\s\([^\)]+\))?$").unwrap());
...
impl ToString for Components {
    fn to_string(&self) -> String {
        let mut v = Vec::with_capacity(self.verbal as usize + self.somatic as usize + self.material.is_some() as usize);
        if self.verbal {
            v.push("V".to_string());
        }
        if self.somatic {
...
impl<'de> Visitor<'de> for ComponentsVisitor {
    ...
    fn visit_str<E>(self, v: &str) -> Result<Self::Value, E> where
        E: Error, {
        v.parse().map_err(|_| E::custom("failed to parse spell components"))
    }
